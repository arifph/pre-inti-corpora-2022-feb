...
public interface UserContactRepository extends JpaRepository<UserContact, String> {
    ...
    @Modifying
    @Query(value = "INSERT INTO UserContact (id, address, userId) values (:id, :address, :userId)", nativeQuery = true)
    void addUserContact(@Param("id") String id, @Param("address") String address, @Param("userId") String userId);
}
